...
def cal_mAP(res_dir, gt_dir, class_path, imagesetfile):
    ...
    detpath = cachedir + '/{:s}.txt'  # 检测结果的路径，{:s}.txt方便后面根据图像名字读取对应的txt文件
...
if __name__ == '__main__':
    res_dir = r'D:\coding\华为目标检测\PyTorch-YOLOv3-ModelArts\cal_mAP\txt'  # 模型预测结果
    gt_dir = r'D:\coding\华为目标检测\trainval\VOC2007\Annotations'  # groud_truth
    class_path = r'D:\coding\华为目标检测\trainval\train_classes.txt'  # 数据集所有类别名的文件
    imagesetfile = r'D:\coding\华为目标检测\PyTorch-YOLOv3-ModelArts\cal_mAP\val_.txt'  # 读取图像名字列表文件
